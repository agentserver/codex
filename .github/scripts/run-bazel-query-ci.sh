...
if [[ "${RUNNER_OS:-}" == "Windows" && $windows_cross_compile -eq 1 && -n "${BUILDBUDDY_API_KEY:-}" ]]; then
  # Keep target-discovery queries on the same platform shape as the following
  # Windows cross build. Platform-specific rc expansion on Windows can override
  # `ci-windows-cross`, so repeat the host platform after configs.
  bazel_query_args+=(--host_platform=//:rbe --shell_executable=/bin/bash)
fi
