...
struct RunExecLikeArgs {
    ...
    target_environment: crate::tools::handlers::ToolCallEnvironment,
        ...
        match payload {
            ToolPayload::Function { arguments } => {
                let ResolvedToolCall {
                    ...
                    cwd,
                } = resolve_tool_call_from_arguments::<ShellToolCallParams>(
                    &turn,
                    &arguments,
            ...
            ToolPayload::LocalShell { params } => {
                let Some(target_environment) =
                    resolve_tool_call_environment(&turn, /*environment_id*/ None)?
                else {
                    return Err(FunctionCallError::RespondToModel(
...
impl ToolHandler for ShellCommandHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        let ResolvedToolCall {
            ...
            environment: target_environment,
            cwd,
        } = resolve_tool_call_from_arguments::<ShellCommandToolCallParams>(
            &turn,
            &arguments,
